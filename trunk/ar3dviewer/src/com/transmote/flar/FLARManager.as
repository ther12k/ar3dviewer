...
//		private var _jitterSuppression:Number = 20;
		...
		/**
		 * Tracking engines may report erratic results under certain conditions.
		 * Jitter suppression eliminates erratic values in X and Y rotation.  
		 * <p>
		 * X/Y rotation changes of more than <code>jitterSuppression</code> are discarded,
		 * and the previous frame's values are used instead.
		 * Set to <code>0</code> to disable jitter suppression.
		 * </p>
		 */
		/*
		public function get jitterSuppression () :Number {
			return this._jitterSuppression;
		}
		public function set jitterSuppression (val:Number) :void {
			if (this.bVerbose) {
				trace("[FLARManager] jitter suppression: "+ val);
			}
			this._jitterSuppression = val;
		}
		*/
...
//			var bJitterDetected:Boolean = false;
			while (i--) {
				...
				// find the activeMarker closest to detectedMarker, within markerUpdateThreshold
				while (j--) {
				...
				// if detectedMarker is within markerUpdateThreshold of an activeMarker,
				// set closestMarker = activeMarker, and update it.
				if (closestMarker) {
					/*
					bJitterDetected = false;
					if (this.jitterSuppression > 0) {
						bJitterDetected = detectedMarker.detectJitter(closestMarker, this.jitterSuppression);
					}
					// only copy values from detected marker to closestMarker if no jitter detected.
					if (!bJitterDetected) {
		...
		private function initFlarSource () :void {
			...
			if (this.configLoader.useProxy) {
				...
				FLARCameraSource(this._flarSource).init(
					...
					this.configLoader.trackerToSourceRatio, this.configLoader.activityThreshold);
			}
		}
		...
		private function onSourceInited (evt:Event) :void {
			Sprite(this._flarSource).removeEventListener(Event.INIT, this.onSourceInited);
			this.onTrackerInited(null);
		}
		
		private function onTrackerInited (evt:Event) :void {
			if (!this._flarSource.source) {
				// source not yet inited; wait until it is to proceed with trackerManager init.
				Sprite(this._flarSource).addEventListener(Event.INIT, this.onSourceInited);
				return;
			}
